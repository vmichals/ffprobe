...
class FFStream:
    ...
    def __init__(self, datalines):
        self.side_data = []
        reading_side_data = False
        for a in datalines:
            if re.match('\[SIDE_DATA\]', a):
                datalines = []
                reading_side_data = True
            elif re.match('\[\/SIDE_DATA\]', a):
                self.side_data.append(FFStreamSideData(datalines))
                datalines = []
                reading_side_data = False
            else:
                if reading_side_data:
                    datalines.append(a)
                else:
    ...
    def frameSize(self):
        ...
        if self.isVideo():
            if self.__dict__['width'] and self.__dict__['height']:
                try:
                    size = (int(self.__dict__['width']), int(
...
class FFStreamSideData(object):
    """
    An object representation of a stream's side data.
    """
    ...
    def __init__(self, datalines):
        ...
        for a in datalines:
            ...
            elif re.match('displaymatrix=', a):
                reading_displaymatrix = True
                self.displaymatrix = []
            else:
                (key, val) = a.strip().split('=')
                self.__dict__[key] = val
